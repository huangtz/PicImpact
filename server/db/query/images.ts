...
export async function fetchServerImagesListByAlbum(
  ...
  return await db.$queryRaw`
    ...
    FROM 
        "pic"."images" AS image
    LEFT JOIN "pic"."images_albums_relation" AS relation
        ON image.id = relation."imageId"
    LEFT JOIN "pic"."albums" AS albums
        ON relation.album_value = albums.album_value
    WHERE 
        image.del = 0
        ${showStatus !== -1 ? Prisma.sql`AND image.show = ${showStatus}` : Prisma.empty}
...
export async function fetchImagesAnalysis():
  ...
  const counts = await db.$queryRaw<[{ images_total: number, images_show: number, cr_total: number, tags_total: number }]>`
    SELECT 
      (SELECT COALESCE(COUNT(*), 0) FROM "pic"."images" WHERE del = 0) as images_total,
      (SELECT COALESCE(COUNT(*), 0) FROM "pic"."images" WHERE del = 0 AND show = 0) as images_show,
      (SELECT COALESCE(COUNT(*), 0) FROM "pic"."albums" WHERE del = 0) as tags_total
  `
